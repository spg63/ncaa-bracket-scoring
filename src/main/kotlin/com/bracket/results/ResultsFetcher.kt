 ...
 *   ?groups=100&limit=100&dates={year}0301-{year}0410
 *
 * groups=100 returns all 67 tournament games (including 4 First Four play-ins, which we skip).
 *
 * ─── Game ID format ──────────────────────────────────────────────────────────
 *   SOUTH_R64_G1 … SOUTH_R64_G8   (8 games per region × 4 regions = 32)
 *   SOUTH_R32_G1 … SOUTH_R32_G4   (4 games per region × 4 regions = 16)
 *   SOUTH_S16_G1 … SOUTH_S16_G2   (2 games per region × 4 regions =  8)
 *   SOUTH_E8_G1                   (1 game  per region × 4 regions =  4)
 *   FF_G1, FF_G2                  (2 Final Four games)
 *   CHAMP                         (1 Championship game)
 *
 * ─── Bracket tree wiring ─────────────────────────────────────────────────────
 *   Within a region, games are sorted by ESPN event ID, then numbered 1…N.
 *   R32 game k  ← R64 game (2k-1)  + R64 game (2k)
 *   S16 game k  ← R32 game (2k-1)  + R32 game (2k)
 *   E8  game 1  ← S16 game 1       + S16 game 2
 *   FF  game k  ← E8  game (2k-1)  + E8  game (2k)   (sorted by event ID)
 *   CHAMP       ← FF_G1            + FF_G2
 */
object ResultsFetcher {
    ...
    // Region order determines global game numbering within each round.
    private val REGION_ORDER = listOf("South", "East", "Midwest", "West")

    // ─── Public API ───────────────────────────────────────────────────────────

    fun fetchTournamentStructure(): TournamentStructure? {
        println("  → Fetching live tournament data from ESPN...")
        val espn = tryFetchESPN()
        if (espn != null) {
            println("  ✓ ESPN data loaded (${espn.games.size} games).")
            return espn
        }

        println("  ⚠ ESPN fetch failed, trying NCAA API...")
        val ncaa = tryFetchNCAA()
        if (ncaa != null) {
            println("  ✓ NCAA data loaded (${ncaa.games.size} games).")
            return ncaa
        }

        println("  ✗ Both sources failed. Check your internet connection.")
        return null
    }
    ...
    private fun parseESPNScoreboard(root: JsonObject): TournamentStructure? {
        ...
        // Raw game data before we assign IDs
        data class RawGame(
            val eventId: String,
            val round: TournamentRound,
            val region: String,          // "South"/"East"/"Midwest"/"West"/"National"
            val team1: Team,
            val team2: Team,
        ...
        // ── Regional rounds (R64 → E8): sort within each region by event ID ──
        ...
        // gameId → index within region-round (for source wiring)
        // We build a map: (region, round) → list of assigned gameIds in order
        val regionRoundGameIds = mutableMapOf<Pair<String, TournamentRound>, List<String>>()

        for (round in regionalRounds) {
            for (region in REGION_ORDER) {
                // Sort games into bracket-position slots; TBD games fill remaining slots
                // in event-ID order. This ensures the bracket tree is correct even when
                // some future games haven't been scheduled yet (teams show as seed 99/TBD).
                ...
                val gameIds = group.mapIndexed { idx, rg ->
                    regionGameId(region, round, idx + 1)
                }
                ...
                group.forEachIndexed { idx, rg ->
                    ...
                    val gameNum = idx + 1  // 1-based

                    // Source games: game k comes from prev-round game (2k-1) and (2k)
        ...
        // ── Final Four ──
        // NCAA bracket structure: South vs East in one semifinal, Midwest vs West in the other.
        // FF_G1 = South champion vs East champion
        // FF_G2 = Midwest champion vs West champion
        ...
        val ffRaw = rawGames
            .filter { it.round == TournamentRound.FINAL_FOUR }
            .sortedBy { it.eventId }
    ...
    private fun get(url: String): String? {
        val request = HttpRequest.newBuilder()
            .uri(URI.create(url))
            .header("User-Agent", "Mozilla/5.0 (compatible; bracket-scorer/1.0)")
    ...
    /** Region-prefixed game ID, e.g. "SOUTH_R64_G1", "FF_G1", "CHAMP" */
    ...
    /** Legacy game ID for NCAA fallback (no region prefix). */
    ...
    /**
     * Returns the bracket position (0-based) of a game within its region and round,
     * based on team seeds. Each round's seed sets are disjoint by bracket quadrant.
     *
     * R64 order:  [1, 8, 5, 4, 6, 3, 7, 2]  (standard NCAA bracket seed order)
     * R32 groups: {1,8,9,16}=0  {4,5,12,13}=1  {3,6,11,14}=2  {2,7,10,15}=3
     * S16 halves: {1,4,5,8,9,12,13,16}=0  {2,3,6,7,10,11,14,15}=1
     * E8:         only 1 game per region, always position 0
     */
    private fun bracketPosition(round: TournamentRound, seed1: Int, seed2: Int): Int {
        ...
        return when (round) {
            ...
            TournamentRound.SWEET_16 -> {
                // Check both seeds — use any non-TBD seed to identify the half
            ...
            else -> 0  // E8 has only 1 game per region
        }
    }
